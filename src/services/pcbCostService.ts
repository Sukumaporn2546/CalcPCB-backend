...
import { Category, MaterialItem, CopperWeightItem, SurfaceFinishItem, SolderMaskItem, SilkscreenItem } from "../enums/materialEnum";
...
interface ICalculateCostBasePriceInput {
    panel_size: { width: number; height: number };
    pcb_size: { width: number; height: number };
    cavity_up: number;
    quantity: number;
    model_name: string;
    material: { copper_weight: string, surface_finish: string, layers: string, base_material: string, thickness: string };
    solder_mask: string,
    legend_silk_screen: string,
    process: {
        CNC_Routing: string
...
export function calculatePCBCost(data: ICalculateCostInput): ICost {
    ...
    const { fixture_charge, express_cost, handling, fly_probe, shipment_cost } = variable_cost;
    ...
    if (models.length <= 1 && models[0]) {
        ...
        if (cavity_up == 1 && parseFloat(copper_weight) > 1) {
            selected = models.find(m => m.cavity_up === 1 && parseFloat(m.material?.copper_weight || '0') > 1);
            price = selected?.cost_in2 ?? 0;
        } else if (cavity_up == 3) {
            selected = models.find(m => m.cavity_up === 3);
            price = quantity <= 1500 ? selected?.cost_in2_many?.q1500 ?? 0 : selected?.cost_in2_many?.q3000 ?? 0;
    ...
    const realCost = calcCostFromSupAndAdmin(allCostPerPCB, allTotalCost, admin_fee.admin_fee_per_pcb, admin_fee.total_admin_fee,quantity);
...
export function calculatePCBCostFromBasePrice(data: ICalculateCostBasePriceInput) {
    const {
        panel_size,
        quantity,
        cavity_up,
    ...
    const baseMaterialPrice = getAveragePrice(Category.MATERIAL, material.base_material);
    const layerPrice = getAveragePrice(Category.LAYER, material.layers)
    const copperWeightPrice = getAveragePrice(Category.COPPER_WEIGHT, material.copper_weight);
    const surfaceFinishPrice = getAveragePrice(Category.SURFACE_FINISH, material.surface_finish);

    const solderMaskItem = solder_mask === "green"
        ? SolderMaskItem.GREEN
        : SolderMaskItem.OTHER_COLORS;
    const solderMaskPrice = getAveragePrice(Category.SOLDER_MASK, solderMaskItem);

    const silkscreenItem = legend_silk_screen === "white"
        ? SilkscreenItem.WHITE
        : SilkscreenItem.OTHER_COLORS;
    const silkscreenPrice = getAveragePrice(Category.SILKSCREEN, silkscreenItem);

    const thicknessPrice = getAveragePrice(Category.THICKNESS, material.thickness);
