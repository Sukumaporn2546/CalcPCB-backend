export function calcSellingPrice(costPerPiece: number, marginPercent: number, quantity: number): {unitPrice: number, totalPrice: number} {

    const totalCost = costPerPiece+(costPerPiece*(marginPercent/100));
    const unitPrice = parseFloat((totalCost).toFixed(2));
