...
import { calculatePCBCost, calculatePCBCostFromBasePrice } from "../services/pcbCostService";
import { calculatePCBPrice } from "../services/pcbPriceService"
import { IPCBinDB } from '../interfaces/model'
import { ICost, IResultCompare, IResultCompareArray } from "../interfaces/resultCompare";
...
import { calcEstimateShipmentCost, getRefWeightPrice, calcShippingAirCost, calcDHLCost, getRate } from "../services/shipmentCalculator";
...
export const updateModePCB = async (req: Request, res: Response) => {
    try {
        ...
        if (Array.isArray(newData.supplier) && newData.supplier.length > 0) {
            ...
            const suppliers = await SupplierModel.find({ sup_name: { $in: supplierNames } });

            if (suppliers.length !== supplierNames.length) {
                const foundNames = suppliers.map(s => s.sup_name);
                const notFound = supplierNames.filter(name => !foundNames.includes(name));
                res.status(404).json({ message: `ไม่พบซัพพลายเออร์ชื่อ: ${notFound.join(', ')}` });
            }
        ...
        const updated = await modelPCBModel.findByIdAndUpdate(id, newData, { new: true });
        if (!updated) {
            res.status(404).json({
                success: false,
...
export const getModelPCB = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        //console.log('pcb', pcb);
        const { material, pcb_size_mm, panel_size_mm, fix_cost, variable_cost, model_name, supplier, cavity_up, solder_mask, legend_silk_screen, quantity, process } = pcb;
...
export const getModelNameFromSup = async (req: Request, res: Response) => {
    try {
        ...
        const modelNames = await PCBModel.find({ supplier: sup_name }).distinct('model_name');
...
export const comparePriceFromSup = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({ message: 'Failed to get data of compare price from sup', error })
    }
}
...
export const calcShipmentCost = async (req: Request, res: Response) => {
    try {
        ...
        const refPrice = getRefWeightPrice(weight, shipping_type, shipping_method, supplier);
        ...
        console.log('Argument for find refPrice: ', shipping_type, shipping_method, supplier);
