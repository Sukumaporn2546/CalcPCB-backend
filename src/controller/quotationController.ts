...
export const updateQuotation = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const allowedFields = [
            "model_name", "supplier", "customer_name", "cost_per_pcb", "total_cost_pcb",
            "unit_price", "total_price", "profit_price", "all_profit_price", "status"
        ];

        const sanitizedBody = Object.fromEntries(
            Object.entries(req.body).filter(([key]) => allowedFields.includes(key))
        );
        const updated = await QuotationRecordModel.findByIdAndUpdate(id, sanitizedBody, { new: true }); //หลังจากอัปเดตเสร็จ ให้คืนค่าข้อมูลใหม่ล่าสุด (ที่ถูกอัปเดตแล้ว)
        if (!updated) {
            res.status(404).json({
                success: false,
                message: 'ไม่พบข้อมูล'
...
export const deleteQuotation = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const deleteModel = await QuotationRecordModel.findOneAndDelete({ _id: id });
        if (!deleteModel) {
            res.status(404).json({
                success: false,
